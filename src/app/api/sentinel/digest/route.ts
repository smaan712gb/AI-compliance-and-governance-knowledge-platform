...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (topEvents.length === 0) {
      return NextResponse.json({ message: "No significant events in last 24h" });
    }
    ...
    // Generate AI digest using the reasoning engine
    ...
    return NextResponse.json({
      status: "completed",
      eventsAnalyzed: topEvents.length,
      categories: categoryBreakdown.map((c) => ({ category: c.category, count: c._count })),
      hotspots: countryBreakdown.map((c) => ({ country: c.countryCode, count: c._count })),
      digest: {
        riskScore: digestAnalysis.riskScore,
        severity: digestAnalysis.severity,
        reasoning: digestAnalysis.reasoning,
        actionableInsights: digestAnalysis.actionableInsights,
      },
      deliveredTo: orgAdmins.length,
    });
  } catch (error) {
    console.error("[sentinel/digest] Error:", error);
    return NextResponse.json(
