...
import { ropaSchema } from "@/lib/validators/privacy-ops";
import { buildROPAPrompt, ROPA_SYSTEM_PROMPT } from "@/lib/ai/privacy-ops-prompts";
  ...
  try {
    ...
    if (!session?.user?.id) {
      return NextResponse.json(
        { success: false, error: { code: "UNAUTHORIZED", message: "You must be signed in to use ROPA generator." } },
        { status: 401 }
      );
    }

    const userId = session.user.id;
    const access = await checkFeatureAccess(userId, "ropa_entry");
    if (!access.allowed) {
      return NextResponse.json(
        {
          success: false,
          error: {
            ...
            message: access.upgradeRequired
              ? `ROPA generator requires a Starter or higher plan. You are on the ${access.tier} tier.`
    ...
    const parsed = ropaSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
    ...
    const prompt = buildROPAPrompt(parsed.data);
      ...
      async start(controller) {
        try {
          ...
          try {
            ...
            if (company) {
              await db.processingActivity.create({
                data: {
                  ...
                  dataSubjectTypes: parsed.data.dataSubjectTypes,
                  recipients: parsed.data.recipients || [],
                  transferMechanisms: parsed.data.transferMechanisms || [],
              ...
              await incrementUsage(company.id, "ropa_entry");
            }
          } catch (dbError) {
    ...
    console.error("[ROPA] Error:", message);
    return NextResponse.json(
      { success: false, error: { code: "INTERNAL_ERROR", message: `Failed to generate ROPA entry: ${message}` } },
      { status: 500 }
    );
  }
}
