...
import { hipaaComplianceSchema } from "@/lib/validators/hipaa-compliance";
import {
  buildHIPAACompliancePrompt,
  HIPAA_COMPLIANCE_SYSTEM_PROMPT,
} from "@/lib/ai/hipaa-compliance-prompts";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const parsed = hipaaComplianceSchema.safeParse(body);
    ...
    const prompt = buildHIPAACompliancePrompt(parsed.data);
      ...
      async start(controller) {
        try {
          const stream = await deepseek.chat.completions.create({
            model: "deepseek-chat",
            messages: [
              { role: "system", content: HIPAA_COMPLIANCE_SYSTEM_PROMPT },
          ...
          try {
            ...
            await db.hIPAAComplianceCheck.create({
              data: {
                userId,
                companyId,
            ...
            console.error("[HIPAA Compliance] Failed to save:", dbError instanceof Error ? dbError.message : String(dbError));
          }
        } catch (streamError) {
          const errMsg = streamError instanceof Error ? streamError.message : String(streamError);
          console.error("[HIPAA Compliance] Stream error:", errMsg);
          controller.enqueue(encoder.encode(`data: ${JSON.stringify({ error: `HIPAA compliance check failed: ${errMsg}` })}\n\n`));
          controller.close();
        }
      },
    });
    ...
    console.error("[HIPAA Compliance] Error:", message);
    return NextResponse.json(
      {
        success: false,
        error: {
          code: "INTERNAL_ERROR",
          message: `Failed to run HIPAA compliance check: ${message}`,
        },
      },
      { status: 500 }
    );
